...
public class SupervisedController {
    ...
    public ResponseEntity postDateset(
            @PathVariable("file-name") String fileName
            , @RequestPart("file") MultipartFile file
    ) throws Exception {
        return new ResponseEntity(mlService.uploadCSV(file, fileName), HttpStatus.CREATED);
    }
    ...
    public ResponseEntity linerRegression(
    ...
    public ResponseEntity getDatasetStat(
            @RequestHeader(value = Constants.USER_SESSION) String sessionId
    ) throws Exception {
